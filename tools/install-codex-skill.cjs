...
console.log("  Use codex-visual-builder-guild on this app. For new sections or important surfaces, first state product intent and page intent: target user, core job, success moment, page job, ideal user route, scary/hard/bloated risks, and what design should make easier. Run it, screenshot desktop, tablet, mobile, and one awkward in-between width, inspect screenshots with Codex App vision, record concrete vision observations by viewport, check whether the user flow has a clear next action and whether reusable patterns follow the art bible/component system, fix the single highest-impact visual issue or flow issue, use at most 1-2 specialist lenses only if screenshots prove they are needed, capture after screenshots with local paths, run the Post-Fix Ruthlessness Check for awkward leftover onboarding, duplicated actions, floating copy, box soup, text-first clutter, and responsive regressions, make one more focused fix if that check reveals a higher-impact visible issue, then finish with the Run Report Contract including product intent, page intent, viewport matrix, state matrix, post-fix ruthlessness, accepted visual change, and automation notes.");
